...
#[derive(Serialize, Deserializa, AsChangeset, Insertable)]
...
impl Employees {
    ...
    pub fn delete(id: i32) -> Result<usize, CustomError> {
        let conn = db::connection()?;
        let res = diesel::delete(employee::table.filter(employee::id.eq(id))).execute(&conn)?;
        Ok(res)
    }
}
